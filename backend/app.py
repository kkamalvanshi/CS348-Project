...
def generate_deployment_report():
    ...
    query = "CALL generate_deployment_report(:start_date, :end_date, :model_type)" if db.engine.dialect.name == 'mysql' else "SELECT * FROM generate_deployment_report(:start_date, :end_date, :model_type)"
    result = db.session.execute(text(query), {'start_date': start_date, 'end_date': end_date, 'model_type': model_type}).fetchall()
    ...
    'server_name': row[1],
    'model_name': row[2],
    'deployment_time': f"{str(row[3])[:-2]}/{str(row[3])[-2:]}" if row[3] else "Not Available"
    } for row in result
    ]
    }
...
def get_top_servers():
    ...
    query = """
        SELECT name AS server_name, COUNT(md.id) AS deployments_count
        FROM server s
        ...
        LIMIT :top_x
    """
    result = db.session.execute(query, {'top_x': top_x}).fetchall()

    top_servers = [{'server_name': row[0], 'deployments_count': row[1]} for row in result]
    return jsonify(top_servers)


@app.route('/reports/model-types-count', methods=['GET'])
def get_model_types_count():
    query = "CALL get_model_types_count()" if db.engine.dialect.name == 'mysql' else "SELECT * FROM get_model_types_count()"
    result = db.session.execute(text(query)).fetchall()

    model_types_count = [{'type': row[0], 'count': row[1]} for row in result]
...
def get_top_models():
    ...
    # Ensure the query correctly joins Model, Version, and ModelDeployment tables
    # and groups by Model.id to aggregate deployment counts
    top_models_query = db.session.query(
    Model.id, Model.name,
    db.func.count(ModelDeployment.version_id).label('deployments_count')
    ).join(Version, ModelDeployment.version_id == Version.id)\
    .join(Model, Version.model_id == Model.id)\
    .group_by(Model.id)\
    .order_by(db.desc('deployments_count'))\
    .limit(top_x)

    top_models = top_models_query.all()

    # Prepare the data for JSON response
    top_models_response = [{
    'model_name': model.name,
    'deployments_count': deployments_count
    } for model, deployments_count in top_models]

    return jsonify(top_models_response), 200
